...
def fix_stanford_coref(stanford_json):
    ...
    for key, coref in stanford_json["corefs"].items():
        ...
        for entity in coref:
            ...
            entity["headWord"] = entity["text"]\
                .split(" ")[head_index - start_index]
...
if __name__ == "__main__":
    ...
    nlp_pipeline = stanza.Pipeline(
        "en", processors=all_setting["props"]["annotators"])
    annotator = partial(nlp_pipeline)
    ...
    for file_path in tqdm.tqdm(input_queue(INPUT_DIR)):
        ...
        file, name, content, rc = process_it(
            annotator, file_path, all_setting["TEXT_MAXLEN"])
        # counting somethings
        cluster_count += 1
        report_count += rc
